...
//		}
	...
	/*
	 * public int Web_tables_text(By tablename, String Excellvalue , String
	 * ExcolName ) { //boolean result = false; int row =-1; WebElement table =
	 * driver.findElement(tablename); List<WebElement> cells =
	 * table.findElements(By.xpath(("tbody/tr/th"))); int colCount = cells.size();
	 * int colnum=1 ; for(int i = 1 ; i<=colCount; i++) { String colname =
	 * table.findElement(By.xpath(("tbody/tr/th["+i+"]"))).getText();
	 * if(colname.equalsIgnoreCase(ExcolName)) { colnum=i; break; } } int rows =
	 * table.findElements(By.xpath(("tbody/tr"))).size(); int rownum = 0; for(int j
	 * =2; j<=rows;j++) { String cellvalue=table.findElement(By.xpath(("tbody/tr["+j
	 * +"]/td["+colnum+"]"))).getText();
	 * 
	 * if(cellvalue.equalsIgnoreCase(Excellvalue)) { rownum =j; break; }
	 * 
	 * }
	 * 
	 * if(rownum>=2) { row= rownum; } return row; }
	 */
	...
	//method to take screenshot with timestamp in ScreenShots folder yyyy-MM-dd_HH-mm-ss 
